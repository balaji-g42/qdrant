...
curl -L -X PUT  "$BASE_URL/collections/$COLLECTION_NAME/index" \
  -H 'Content-Type: application/json' "${qdrant_host_headers[@]}" \
  --fail -s \
  --data-raw '{
      "field_name": "city",
...
curl --fail -s "$BASE_URL/collections/$COLLECTION_NAME" "${qdrant_host_headers[@]}" | jq

# insert points
curl -L -X PUT "$BASE_URL/collections/$COLLECTION_NAME/points?wait=true" \
  -H 'Content-Type: application/json' "${qdrant_host_headers[@]}" \
  --fail -s \
  --data-raw '{
      "points": [
...
# retrieve point
curl -L -X GET "$BASE_URL/collections/$COLLECTION_NAME/points/2" \
  -H 'Content-Type: application/json' "${qdrant_host_headers[@]}" \
  --fail -s | jq

# retrieve points
curl -L -X POST "$BASE_URL/collections/$COLLECTION_NAME/points" \
  -H 'Content-Type: application/json' "${qdrant_host_headers[@]}" \
  --fail -s \
  --data-raw '{
      "ids": [1, 2],
...
SAVED_POINTS_COUNT=$(curl --fail -s "$BASE_URL/collections/$COLLECTION_NAME" "${qdrant_host_headers[@]}" | jq '.result.points_count')
...
# search points
curl -L -X POST "$BASE_URL/collections/$COLLECTION_NAME/points/search" \
  -H 'Content-Type: application/json' "${qdrant_host_headers[@]}" \
  --fail -s \
  --data-raw '{
        "vector": {
...
# search points batch
curl -L -X POST "$BASE_URL/collections/$COLLECTION_NAME/points/search/batch" \
  -H 'Content-Type: application/json' "${qdrant_host_headers[@]}" \
  --fail -s \
  --data-raw '{
    "searches": [
